...
export default function WebDevPage({ setPage }) {
  ...
  return (
    <main className="service-detail-page" style={{ padding: "120px 24px 80px", maxWidth: 1000, margin: "0 auto", color: "var(--text, #1A1208)" }}>
      ...
      <span className="tag" style={{ display: "inline-flex", marginBottom: 12 }}>Core Expertise</span>
      <h1 style={{ fontSize: "clamp(2.2rem, 5vw, 3.5rem)", fontFamily: "var(--fd)", fontWeight: 800, color: "var(--text, #1A1208)", marginBottom: 20 }}>
        Website Design & <span style={{ color: "var(--gold, #D97706)" }}>Web Development</span>
      </h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24, marginBottom: 60 }}>
        <div className="card" style={{ background: "var(--card, #FFFDF9)", border: "1px solid var(--border)", borderRadius: 16, padding: 28 }}>
          <h3 style={{ color: "var(--text, #1A1208)", fontSize: 20, marginBottom: 12, fontFamily: "var(--fd)" }}>🚀 Shipped in 7 Days</h3>
          <p style={{ color: "var(--muted, #665843)", fontSize: 14, lineHeight: 1.65 }}>We don't drag projects out over months. Clear milestones and rapid execution mean your business goes live fast.</p>
        </div>
        <div className="card" style={{ background: "var(--card, #FFFDF9)", border: "1px solid var(--border)", borderRadius: 16, padding: 28 }}>
          <h3 style={{ color: "var(--text, #1A1208)", fontSize: 20, marginBottom: 12, fontFamily: "var(--fd)" }}>📱 Mobile-First UX</h3>
          <p style={{ color: "var(--muted, #665843)", fontSize: 14, lineHeight: 1.65 }}>Over 80% of your visitors browse on smartphones. Every page is pixel-perfect and optimized for touch devices.</p>
        </div>
        <div className="card" style={{ background: "var(--card, #FFFDF9)", border: "1px solid var(--border)", borderRadius: 16, padding: 28 }}>
          <h3 style={{ color: "var(--text, #1A1208)", fontSize: 20, marginBottom: 12, fontFamily: "var(--fd)" }}>🎯 Built for Conversions</h3>
          <p style={{ color: "var(--muted, #665843)", fontSize: 14, lineHeight: 1.65 }}>WhatsApp lead capture, CRM-ready contact forms, and strategic CTAs convert passive traffic into real customer enquiries.</p>
        </div>
      </div>

      <section style={{ marginBottom: 60 }}>
        <h2 style={{ fontSize: 28, color: "var(--text, #1A1208)", marginBottom: 24, fontFamily: "var(--fd)" }}>Our Web Development Process</h2>
      ...
      <div style={{ textAlign: "center", background: "var(--card, #FFFDF9)", border: "1px solid rgba(217,119,6,0.3)", padding: 40, borderRadius: 20, boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
        <h3 style={{ fontSize: 26, color: "var(--text, #1A1208)", marginBottom: 12, fontFamily: "var(--fd)" }}>Ready for a website that actually drives growth?</h3>
        <p style={{ color: "var(--muted, #665843)", marginBottom: 24 }}>Book a free 30-minute strategy call or request a custom project quote today.</p>
